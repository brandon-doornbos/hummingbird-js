window.HB = (function() {
	...
	return {
		...
		init: function(width, height, options) {
			...
			} else HB.canvas = options["canvas"], HB.gl = HB.canvas.getContext('webgl2');
			...
			if(HB.gl === null) {
				...
				HB.canvas.center = HB.Vec2.new(HB.canvas.width/2, HB.canvas.height/2);
		...
		resizeCanvas: function(width, height) {
			...
			HB.Vec2.set(HB.canvas.center, HB.canvas.width/2, HB.canvas.height/2);
		...
		Batch: class{
			...
			// 		if((HB.batch.vertexCount + 3) >= HB.maxVertexCount || (HB.batch.indexCount + 3) >= HB.maxIndexCount) HB.batch.flush();

			// 		const start = HB.batch.vertexCount*HB.vertexStride;
			// 		HB.vertices[start   ] = points[startIndex][0];
			// 		HB.vertices[start+1 ] = points[startIndex][1];
			...
			// 		HB.vertices[start+8 ] = 1;
			// 		HB.vertices[start+9 ] = 0;
			// 		HB.vertices[start+10] = 0;
			// 		HB.vertices[start+11] = points[startIndex+1][0];
			// 		HB.vertices[start+12] = points[startIndex+1][1];
			...
			// 		HB.vertices[start+18] = 0.5;
			// 		HB.vertices[start+19] = 0.5;
			// 		HB.vertices[start+20] = 0;
			// 		HB.vertices[start+21] = 0;
			// 		HB.vertices[start+22] = points[startIndex+2][0];
			// 		HB.vertices[start+23] = points[startIndex+2][1];
			...
			// 		HB.indices[HB.batch.indexCount  ] = HB.batch.vertexCount;
			// 		HB.indices[HB.batch.indexCount+1] = HB.batch.vertexCount+1;
			// 		HB.indices[HB.batch.indexCount+2] = HB.batch.vertexCount+2;

			// 		HB.batch.vertexCount += 3, HB.batch.indexCount += 3;
			// 	}
			// }
		...
		Mat4: class{
			...
			static rotate(out, matrix, up, angle) {
				...
				HB.Mat4.multMat4(out, matrix, [1-yy-zz, yx+wz, zx-wy, 0, yx-wz, 1-xx-zz, zy+wx, 0, zx+wy, zy+wx, 1-xx-yy, 0, 0, 0, 0, 1]);
			}
